...
import { MessageCircle, Send, Scale, Globe2, Volume2, VolumeX } from 'lucide-react';
...
function App() {
  ...
  const formatResponse = (text: string) => {
    return text
      ...
      .replace(/\n\s*[-•]\s*/g, '\n') // Convert bullet points to simple newlines
      .replace(/\(\s*([^)]+)\s*\)/g, '$1') // Remove parentheses but keep their content
      .replace(/\[\s*([^\]]+)\s*\]/g, '$1') // Remove square brackets but keep their content
      ...
      <header className="bg-[#1a365d] text-white py-8">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-center gap-3 mb-4">
            <Scale size={40} />
            <h1 className="text-4xl font-bold">Dr. Julio Campos Machado</h1>
          </div>
          <div className="flex items-center justify-center gap-2">
            <Globe2 size={24} />
            <p className="text-xl">Advogado Global - Especialista em Direito Internacional</p>
          </div>
        </div>
      </header>
